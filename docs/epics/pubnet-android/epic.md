...
| 11 | Cellular performance advice + dual-network tabs | feature | 8 | planned | tbd | none |
...
- Tickets 6–11 follow the skeleton in any order; 6, 7, 9 and 11 each open with a
  ...
  enhancement (DoH against the device trust store instead of the bundled Mozilla
  roots), not a blocker — the skeleton's DoH probe already works via
  `webpki-roots` (`crate::tls`). Ticket 10 extends the snapshot to cellular so
  the app is useful (and not misleading) off Wi-Fi; it touches the shared
  `InterfaceKind` type and the snapshot spec. Ticket 11 (needs 10) adds the
  cellular performance-advice surface and, when Wi-Fi + cellular are up at once,
  per-network audit tabs — see `docs/context/cellular-mobile-network.md`.
