...
- **Personal data is scrubbed, with joke stand-ins.** Nothing committed or posted
  ...
  (macOS emits that itself): plain placeholders by default, and a joke name written by
  the dev only where the hidden thing is meaningful (a person, the network under
  study). `capture.sh` scrubs fixtures via `scrub.sh`, which fails the capture rather
...
- [docs/decisions/](docs/decisions/) — why key architectural and technology choices
  ...
  - [2026-09-24-scrub-personal-data.md](docs/decisions/2026-09-24-scrub-personal-data.md): personal data scrubbed from everything committed or posted, with context-themed joke stand-ins; `scrub.sh` does it at capture time with default-deny verification
